...
- OOAD는 두 가지 주요 구성 요소, 즉 OOA(객체지향 분석)와 OOD(객체지향 설계)로 나누어진다.
  - `객체지향 분석 (OOA)`
    ...
    - 이 과정은 시스템이 무엇을 해야 하는지(What)를 정의하는 것에 초점을 맞춘다.
  - `객체지향 설계 (OOD)`
...
> '유스케이스 중심으로 개발을 진행한다'는 말은 실제 사용자의 시나리오를 기반으로 시스템이 어떻게 작동해야 하는지를 정의하는 use case를 개발 과정의 핵심으로 삼는다는 의미이다. 유스케이스를 통해 개발자들은 사용자가 시스템과 상호작용하는 구체적인 방법을 이해할 수 있으며, 이것은 개발하는 소프트웨어가 실제로 사용될 때의 상황을 반영하는 데 도움이 된다.
...
- 에자일과 UP의 차이
  1. 문서화와 고객 협력의 차이:
    - Agile: 작동하는 소프트웨어와 고객과의 지속적인 협력에 더 큰 가치를 두며, 공식적인 문서보다는 제품의 실용성과 고객 만족을 우선시한다.
    - UP: 구조화된 접근과 철저한 문서화를 통해 프로젝트의 각 단계를 관리하며, 복잡하고 규모가 큰 시스템의 개발에 적합한 프레임워크를 제공한다.
  2. 개발 사이클과 반복의 차이:
    - Agile: 짧은 개발 사이클(스프린트)을 사용하여 빠른 피드백을 통해 신속하게 반응하고, 지속적인 개선을 추구한다. 이는 변화에 빠르게 적응하고 고객의 최신 요구를 반영할 수 있는 능력을 향상시킨다.
    - UP: 상대적으로 긴 반복 주기를 통해 각 단계에서 요구되는 더 많은 구조화된 활동과 산출물을 관리한다. 이는 더 큰 규모의 프로젝트나 복잡한 시스템 개발에 필요한 체계적 접근을 보장한다.
